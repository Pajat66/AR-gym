...
import numpy as np
import time
from modules.pose_module4 import poseDetector  # 修改为模块化路径
...
def run_barbell_sit_right():
    """运行右侧杠铃坐姿功能"""
    speak("Starting barbell sit right")
    ...
    while True:
        ...
        if len(lmList) >= 16:
            ...
            # 关节保护提示
            if angle < 45:
                img = putChineseText(img, "膝盖过伸警告！", (layout["angle_pos"][0], layout["angle_pos"][1]+100), 30, (255, 0, 0))
